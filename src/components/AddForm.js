...
const AddForm = (props) => {
    ...
    const { fetchError, addSmurf } = props
    ...
    const handleSubmit = e => {
        ...
        if (state.name === "" || state.position === "" || state.nickname === "") {
            fetchError('All fields required')
        } else {
            addSmurf(state)
        }
    }
    ...
    return (<section>
        ...
        <form onSubmit={handleSubmit}>
            ...
            {
                props.errorMes && <div data-testid="errorAlert" className="alert alert-danger" role="alert">Error: {props.errorMes}</div>
            }
